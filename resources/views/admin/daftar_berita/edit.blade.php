    ...
    <div class="bg-white rounded-xl shadow-xl p-8 w-full my-2">
        ...
        <form action="{{ route('admin.daftar-berita.update',$berita->id) }}" class="flex flex-col space-y-4  text-gray-600 " method="POST" enctype="multipart/form-data">
            ...
            <div>
                ...
                <input name="judul" type="text" id="judul" value="{{ $berita->judul }}" class="form control input input-bordered w-full mt-2 hover:ring-2 ps-4" placeholder="Judul" required>
                @error('judul')
            ...
            <div>
                ...
                <textarea name="isi" id="isi" class="textarea textarea-bordered text-sm w-full mt-2 ps-4" rows="8" placeholder="Isi berita" required>{{ $berita->isi }}</textarea>
                @error('isi')
                    <span class="text-xs text-error">{{ $message }}</span>
                @enderror
            </div>
